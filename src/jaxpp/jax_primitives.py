...
def all_reduce_abstract_eval(arg, shardings_by_mpmd_idx):
    return arg


all_reduce_p.def_abstract_eval(all_reduce_abstract_eval)
...
class ShardingStore:
    ...
    def shardings(self) -> list:
        ...
        if len(self._shardings) > 0 and not self._called_at_least_once:
            raise TypeError("Shardings can be inspected only after compiling the jaxpr")
