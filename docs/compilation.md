...
* With **explicit** (`Explicit`) axes, the shardings are carried in the
  ...
  skipped entirely (see [Explicit sharding](#explicit-sharding)).
...
The cache never changes results, but it can still miss when two structurally
identical tasks are not recognized as equivalent. The dedup
...
For example, in an `add_one` kernel called twice (`interpret=True` so it runs
on any backend), both call sites reference one shared kernel jaxpr, printed
once as a `let` binding (note `jaxpr=add_one` on both calls) rather than
inlining the kernel body twice:
...
call_1, call_2, _ = jaxpr.eqns
assert call_1.params["jaxpr"] is call_2.params["jaxpr"]  # one shared jaxpr
```
...
[explicit sharding mode](https://docs.jax.dev/en/latest/parallel.html), also
